...
const SUPABASE_PK = process.env.SUPABASE_PK || "id"; // primary key for cursor paging
...
const FTP_SECURE = /^(1|true|yes)$/i.test(process.env.ICSOURCE_FTPS || "false");
const REMOTE_DIR = (process.env.ICSOURCE_REMOTE_DIR || "").trim();
...
const QTY_FACTOR   = Number(process.env.ICSOURCE_QTY_FACTOR || "0.8");
...
const PAGE = Number(process.env.SUPABASE_PAGE || "1000"); // chunk size
...
function buildRows(raw) {
  ...
  if (!AGGREGATE) {
    ...
    for (const r of usable) {
      ...
      if (q > 0) { rows.push({ part_number: pn, quantity: q }); total += q; }
    }
...
(async () => {
  try {
    ...
    if (DRY_RUN) { info(`CSV ready at: ${csvPath}`); return; }
